<?php
  ...
  */
  ...
  function find_menu_by_id($id, $options=[]) {
    global $db;
    ...
    mysqli_free_result($result);
    return $menu; // returns an assoc. array
  }
  ...
  */
  ...
  function validate_menu($menu) {
    $errors = [];


    // menu_name
    if(is_blank($menu['menu_name'])) {
      $errors[] = "Menu tidak boleh kosong.";
    } elseif(!has_length($menu['menu_name'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Menu harus berisi 2 - 255 karakter.";
    }
    $current_id = $menu['id'] ?? '0';
    if(!has_unique_menu_menu_name($menu['menu_name'], $current_id)) {
      $errors[] = "Menu harus unik.";
    }

    // visible
    
    // Make sure we are working with a string
    $visible_str = (string) $menu['visible'];
    if(!has_inclusion_of($visible_str, ["0","1"])) {
      $errors[] = "Visible must be true or false.";
    }

    // content
    if(is_blank($menu['content'])) {
      $errors[] = "Content cannot be blank.";
    }
  ...
  function insert_menu($menu) {
    ...
    $sql .= "(menu_name, visible, content) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $menu['menu_name']) . "',";
    $sql .= "'" . db_escape($db, $menu['visible']) . "',";
    $sql .= "'" . db_escape($db, $menu['content']) . "'";
    $sql .= ")";
    $result = mysqli_query($db, $sql);
    // For INSERT statements, $result is true/false
    if($result) {
  ...
  /*function update_page($page) {
    ...
    $sql .= "menu_name='" . db_escape($db, $menu['menu_name']) . "', ";
    $sql .= "visible='" . db_escape($db, $menu['visible']) . "', ";
    $sql .= "content='" . db_escape($db, $menu['content']) . "' ";
    $sql .= "WHERE id='" . db_escape($db, $menu['id']) . "' ";
    $sql .= "LIMIT 1";
...
` */
  ...
  function validate_user($user, $options=[]) {
    ...
    } elseif (!has_length($user['address'], array('min' => 25, 'max' => 255))) {
      $errors[] = "Address must correct.";
    }
	
	if(is_blank($user['phone'])) {
      $errors[] = "Phone cannot be blank.";
    } elseif (!has_length($user['phone'], array('min' => 10, 'max' => 15))) {
      $errors[] = "Phone must correct.";
    }
  ...
  function insert_user($user) {
    ...
    $sql .= "(first_name, last_name, email, username,address,phone, hashed_password) ";
    $sql .= "VALUES (";
